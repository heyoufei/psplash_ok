...
psplash_fb_new (int angle)
{
  ...
  if (fb->red_offset == 11 && fb->red_length == 5 &&
	...
	printf("fb->real_width = %d, fb->real_height = %d, fb->bpp = %d, fb->rgbmode = %d\n", fb->real_width, fb->real_height, fb->bpp, fb->rgbmode);
